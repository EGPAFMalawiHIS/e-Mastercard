<template>
  <div id="consistency_check">
    <div v-for="text in this.report_consistency_string">
      {{ text }}
    </div>
  </div>
</template>
    ...
    startValidating() {
      for (var i = 0; i < this.dataparams.length; i++) {
        ...
        if (this.dataparams[i].name == "total_registered")
          this.newly_registered_quarterly = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "pregnant_females_all_ages")
          this.pregnant_females_all_ages = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "cum_pregnant_females_all_ages")
          this.cum_pregnant_females_all_ages = parseInt(
            this.dataparams[i].contents
          );

        if (
          this.dataparams[i].name ==
          "children_below_24_months_at_art_initiation"
        )
          this.children_below_24_months_at_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.cum_children_below_24_months_at_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.children_24_months_14_years_at_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.cum_children_24_months_14_years_at_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "cum_adults_at_art_initiation")
          this.cum_adults_at_art_initiation = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "presumed_severe_hiv_disease_in_infants")
          this.presumed_severe_hiv_disease_in_infants = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.cum_presumed_severe_hiv_disease_in_infants = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "confirmed_hiv_infection_in_infants_pcr")
          this.confirmed_hiv_infection_in_infants_pcr = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.cum_confirmed_hiv_infection_in_infants_pcr = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "quarterly_children_12_59_months")
          this.quarterly_children_12_59_months = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "cum_children_12_59_months")
          this.cum_children_12_59_months = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "cum_breastfeeding_mothers")
          this.cum_breastfeeding_mothers = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          this.dataparams[i].name == "unknown_other_reason_outside_guidelines"
        )
          this.unknown_other_reason_outside_guidelines = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.cum_unknown_other_reason_outside_guidelines = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "tb_within_the_last_two_years")
          this.tb_within_the_last_two_years = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "cum_tb_within_the_last_two_years")
          this.cum_tb_within_the_last_two_years = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "cum_current_episode_of_tb")
          this.cum_current_episode_of_tb = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.died_within_the_1st_month_of_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.died_within_the_2nd_month_of_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.died_within_the_3rd_month_of_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.died_after_the_3rd_month_of_art_initiation = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "total_breastfeeding_women")
          this.total_breastfeeding_women = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.tb_confirmed_currently_not_yet_on_tb_treatment = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "tb_confirmed_on_tb_treatment")
          this.tb_confirmed_on_tb_treatment = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "total_patients_without_side_effects")
          this.total_patients_without_side_effects = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "total_patients_with_side_effects")
          this.total_patients_with_side_effects = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.patients_with_0_6_doses_missed_at_their_last_visit = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (
          ...
          this.patients_with_7_plus_doses_missed_at_their_last_visit = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "patients_with_unknown_adhrence")
          this.patients_with_unknown_adhrence = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "total_patients_on_arvs_and_cpt")
          this.total_patients_on_arvs_and_cpt = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "total_patients_on_arvs_and_ipt")
          this.total_patients_on_arvs_and_ipt = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "total_patients_on_family_planning")
          this.total_patients_on_family_planning = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "total_patients_with_screened_bp")
          this.total_patients_with_screened_bp = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "males_initiated_on_art_first_time")
          this.males_initiated_on_art_first_time = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "cum_males_initiated_on_art_first_time")
          this.cum_males_initiated_on_art_first_time = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "initial_pregnant_females_all_ages")
          this.initial_pregnant_females_all_ages = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "cum_initial_pregnant_females_all_ages")
          this.cum_initial_pregnant_females_all_ages = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "initial_non_pregnant_females_all_ages")
          this.initial_non_pregnant_females_all_ages = parseInt(
            this.dataparams[i].contents
          );

        if (
          this.dataparams[i].name == "cum_initial_non_pregnant_females_all_ages"
        )
          this.cum_initial_non_pregnant_females_all_ages = parseInt(
            this.dataparams[i].contents
          );
        ...
        if (this.dataparams[i].name == "initiated_on_art_first_time")
          this.initiated_on_art_first_time = parseInt(
            this.dataparams[i].contents
          );

        if (this.dataparams[i].name == "cum_initiated_on_art_first_time")
          this.cum_initiated_on_art_first_time = parseInt(
            this.dataparams[i].contents
          );
      }
    },
    validateReport() {
      if (this.newly_registered_cum < 1) return;
